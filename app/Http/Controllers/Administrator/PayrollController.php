...
class PayrollController extends Controller
{
    ...
    public function index()
    {
        ...
        if(count(request()->all())) {
            ...
            \Session::put('payroll_type', request()->payroll_type);
            ...
            \Session::put('employee_resign', request()->employee_resign);
            \Session::put('year', request()->year);
        }

        $is_calculate       = \Session::get('is_calculate');
        $payroll_type       = \Session::get('payroll_type');
        ...
        $employee_resign    = \Session::get('employee_resign');
...
//                    return $this->downloadExcel($result->whereIn('user_id', request()->user_id)->get(),$month,$year);
                    ...
                    if($result->count()==0)
                        return redirect()->route('administrator.payroll.index')->with('message-error', 'No data!');
            ...
            if(request()->action == 'spt')
            {
                if(!isset(request()->user_id))
                    return redirect()->route('administrator.payroll.index')->with('message-error', 'Payroll item required.');
    ...
    public function buktiPotong()
    {
        ...
        if (!$check) {
            return redirect()->route('administrator.payroll.index')->with('message-error', 'Payroll is not define/locked yet!');
        } else {
...
//                    $payroll = Payroll::where('id', PayrollHistory::where('id', $dataHistory->id)->first()->payroll_id)->update(['is_lock' => 1]);
    ...
    public function downloadExcel($data,$month,$year)
    {
        ...
        foreach($data as $k =>  $item)
        {
            ...
            $params[$k][]['Payroll Cycle']    = ($payroll_cycle = get_user_payroll_cycle('payroll', $item->user->id))->label ?: 'Default';
            $params[$k][]['Payroll Cycle Start'] = $payroll_cycle->start_date;
            $params[$k][]['Payroll Cycle End'] = $payroll_cycle->end_date;
            $params[$k][]['Attendance Cycle'] = ($attendance_cycle = get_user_payroll_cycle('attendance', $item->user->id))->label ?: 'Default';
            $params[$k][]['Attendance Cycle Start'] = $attendance_cycle->start_date;
            $params[$k][]['Attendance Cycle End'] = $attendance_cycle->end_date;
    ...
    public function create()
    {
        ...
        $year               = \Session::get('year');
        if(!empty($year) && empty($month)){
            return redirect()->route('administrator.payroll.index')->with('message-error','Select year and month first!');
        }
...
//        if((!isset($request->salary) || empty($request->salary)) && (!isset($request->salary) || empty($request->salary)) || empty($request->user_id)) {
        if(empty($request->user_id)) {
            ...
            if (!$user) {
                return redirect()->route('administrator.payroll.create')->with('message-error', __('payroll.message-employee-not-found'));
            } else {
...
//        $temp->bpjs_ketenagakerjaan_company     = replace_idr($request->bpjs_ketenagakerjaan_company);
        ...
        if($monthly)
        {
            return redirect()->route('administrator.payroll.detail-history', $id)->with('message-success', __('general.message-data-saved-success'));
        }
        else
        {
...
//            $burden_allow = 5 * ($temp->salary + $earnings + $bpjspenambahan + $temp->bonus) / 100;
            ...
            // $burdenAllowTHR = 0;
            // if (!empty(\Session::get('month')) && !empty(\Session::get('year')) && \Session::get('month') == 12 && $item->user->id) {
...
//                $temp->is_calculate                 = 1;
    ...
    public function sendsubmitpayslip($year,$month) {
        ...
        if($count>0)
            return redirect()->route('administrator.payroll.index')->with('message-success', $count.' Pay Slip(s) Send successfully');
        else
            return redirect()->route('administrator.payroll.index')->with('message-error', 'Failed to send checked payslip!');
    }
    ...
    public function tempImport(Request $request)
    {
        ...
        if($request->hasFile('file'))
        {
            ...
            if ($errors != '') {
                return redirect()->route('administrator.payroll.index')->with('message-error', 'Region '.$errors.' no found!');
            }
            ...
            foreach($rows as $key => $row)
            {
                ...
                $user = User::where('nik', $nik)
                    ->leftJoin('payroll_cycle', function($join) {
                        $join->on('users.project_id', '=', 'payroll_cycle.project_id');
                        $join->where('payroll_cycle.key_name', '=', 'payroll');
                    })
                    ->select('users.id', 'users.join_date', 'users.non_active_date', 'users.payroll_umr_id', 'payroll_cycle.start_date', 'payroll_cycle.end_date')
                    ->first();
                if($user) {
                    ...
                    if(!empty($year) && !empty($month)) {
                        ...
                        if ($user->start_date && $user->end_date) {
                            $end_date = fix_date($user->end_date, $month, $year);
        
                            // Start bulan yang sama
                            if ($user->start_date < $user->end_date) {
                                $start_date = fix_date($user->start_date, $month, $year);
                            }
                            // Start bulan sebelumnya
                            else {
                                $prev = get_previous_month($month, $year);
                                $start_date = fix_date($user->start_date, $prev['month'], $prev['year']);
                            }
...
//                    }
            ...
            return redirect()->route('administrator.payroll.index')->with('message-success', 'Data Payroll '.$monthly.' successfully imported!');
        }
    }
