...
class ExitInterview extends Model
{
    ...
    public function assetsApproval($user_id)
    {
        return $this->hasMany('App\Models\ExitInterviewAssets', 'exit_interview_id', 'id')->where(function($q) use ($user_id){
            $q->join('asset as a','exit_interview_assets.asset_id','=','a.id')
              ->join('asset_type as at','a.asset_type_id','=','at.id')
              ->whereInRaw('(SELECT sc.nama_approval from setting_approval_clearance sc where sc.user_id = ?)',$user_id);
        });
    }
