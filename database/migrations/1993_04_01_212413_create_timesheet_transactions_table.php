...
class CreateTimesheetTransactionsTable extends Migration {
	...
	public function up()
	{
		...
		{
			$table->increments('id');
			$table->integer('timesheet_period_id')->unsigned()->nullable()->index('timesheet_transactions_timesheet_period_id_foreign');
			$table->integer('timesheet_category_id')->unsigned()->nullable()->index('timesheet_transactions_timesheet_category_id_foreign');
			$table->string('timesheet_category_name', 191)->nullable();
			$table->integer('timesheet_activity_id')->unsigned()->nullable()->index('timesheet_transactions_timesheet_activity_id_foreign');
	...
	{
		Schema::drop('timesheet_transactions');
	}
