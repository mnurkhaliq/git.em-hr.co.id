...
class CreateEventCutiDaily extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        DB::unprepared("CREATE DEFINER=`".config('database.connections.mysql.username')."`@`".config('database.connections.mysql.host')."` EVENT IF NOT EXISTS `Event Cuti Daily` ON SCHEDULE EVERY 1 DAY STARTS '2020-09-05 00:00:00' ENDS '2021-07-31 00:00:00' ON COMPLETION NOT PRESERVE ENABLE DO BEGIN
...
where c.master_cuti_type_id=4 and DAY(now()) ='1' and c.iscarryforward=0 and c.cutoffmonth is not null;

INSERT INTO `cutilog` (`usercuti_id`, `update_status`, `date_update`) select a.id ,'Success Monthly' ,now() From user_cuti a inner join users b on a.user_id = b.id inner join cuti c on a.cuti_id = c.id where   c.master_cuti_type_id=4 and DAY(now()) ='1' and c.iscarryforward=0 and c.cutoffmonth is not null;
...
update user_cuti a left join cuti c on a.cuti_id = c.id set a.sisa_cuti=(a.sisa_cuti-1),a.cuti_terpakai=(a.cuti_terpakai+1)
where c.jenis_cuti ='Annual Leave' and c.jenis_cuti is NOT null and EXISTS (select * from cuti_bersama WHERE impacttoleave=1 and DAY(now()) =day(dari_tanggal) and month(now())=month(dari_tanggal) and YEAR(now())=YEAR(dari_tanggal) );

INSERT INTO `cutilog` (`usercuti_id`, `update_status`, `date_update`) select a.id ,'Collective Leave' ,now() From user_cuti a left join cuti c on a.cuti_id = c.id 
where c.jenis_cuti ='Annual Leave' and c.jenis_cuti is NOT null and EXISTS (select * from cuti_bersama WHERE impacttoleave=1 and DAY(now()) =day(dari_tanggal) and month(now())=month(dari_tanggal) and YEAR(now())=YEAR(dari_tanggal) );

INSERT INTO `cuti_karyawan` (`user_id`, `jenis_cuti`, `tanggal_cuti_start`,`tanggal_cuti_end`,`keperluan`,`backup_user_id`,`status`,`created_at`,`updated_at`,`total_cuti`,`temp_kuota`,`temp_cuti_terpakai`,`temp_sisa_cuti`) select a.user_id,a.cuti_id,cast(now() as date),cast(now() as date) ,z.description,a.user_id,2,now(),now(),1,a.kuota,a.cuti_terpakai,a.sisa_cuti From user_cuti a left join cuti c on a.cuti_id = c.id 
left join cuti_bersama z on z.dari_tanggal = cast(now() as date)
where c.jenis_cuti ='Annual Leave' and c.jenis_cuti is NOT null and EXISTS (select * from cuti_bersama x WHERE x.impacttoleave=1 and DAY(now()) =day(x.dari_tanggal) and month(now())=month(x.dari_tanggal) and YEAR(now())=YEAR(x.dari_tanggal) );
