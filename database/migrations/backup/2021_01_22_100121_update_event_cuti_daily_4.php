...
class UpdateEventCutiDaily4 extends Migration
{
    /**
     * Run the migrations.
     *
...
where c.master_cuti_type_id=4 and DAY(now()) ='2' and c.cutoffmonth is not null;
...
update user_cuti a left join cuti c on a.cuti_id = c.id left join users u on a.user_id = u.id left join cuti_karyawan ck on ck.user_id = u.id left join cuti_karyawan_dates ckd on ckd.cuti_karyawan_id = ck.id left join shift s on u.shift_id = s.id left join shift_detail sd on sd.shift_id = s.id set a.sisa_cuti=(a.sisa_cuti-1),a.cuti_terpakai=(a.cuti_terpakai+1)
where c.jenis_cuti ='Annual Leave' and s.is_collective = 0 and DAYNAME(now()) = sd.day and sd.day is NOT NULL and c.jenis_cuti is NOT null and ckd.id is NULL and EXISTS (select * from cuti_bersama WHERE impacttoleave=1 and DAY(now()) =day(dari_tanggal) and month(now())=month(dari_tanggal) and YEAR(now())=YEAR(dari_tanggal) );

INSERT INTO `cutilog` (`usercuti_id`, `update_status`, `date_update`) select a.id ,'Collective Leave' ,now() From user_cuti a left join cuti c on a.cuti_id = c.id left join users u on a.user_id = u.id left join cuti_karyawan ck on ck.user_id = u.id left join cuti_karyawan_dates ckd on ckd.cuti_karyawan_id = ck.id left join shift s on u.shift_id = s.id left join shift_detail sd on sd.shift_id = s.id 
where c.jenis_cuti ='Annual Leave' and s.is_collective = 0 and DAYNAME(now()) = sd.day and sd.day is NOT NULL and c.jenis_cuti is NOT null and ckd.id is NULL and EXISTS (select * from cuti_bersama WHERE impacttoleave=1 and DAY(now()) =day(dari_tanggal) and month(now())=month(dari_tanggal) and YEAR(now())=YEAR(dari_tanggal) );

INSERT INTO `cuti_karyawan` (`user_id`, `jenis_cuti`, `tanggal_cuti_start`,`tanggal_cuti_end`,`keperluan`,`backup_user_id`,`status`,`created_at`,`updated_at`,`total_cuti`,`temp_kuota`,`temp_cuti_terpakai`,`temp_sisa_cuti`) select a.user_id,a.cuti_id,cast(now() as date),cast(now() as date) ,z.description,a.user_id,2,now(),now(),1,a.kuota,a.cuti_terpakai,a.sisa_cuti From user_cuti a left join cuti c on a.cuti_id = c.id left join users u on a.user_id = u.id left join cuti_karyawan ck on ck.user_id = u.id left join cuti_karyawan_dates ckd on ckd.cuti_karyawan_id = ck.id left join shift s on u.shift_id = s.id left join shift_detail sd on sd.shift_id = s.id 
left join cuti_bersama z on z.dari_tanggal = cast(now() as date)
where c.jenis_cuti ='Annual Leave' and s.is_collective = 0 and DAYNAME(now()) = sd.day and sd.day is NOT NULL and c.jenis_cuti is NOT null and ckd.id is NULL and EXISTS (select * from cuti_bersama x WHERE x.impacttoleave=1 and DAY(now()) =day(x.dari_tanggal) and month(now())=month(x.dari_tanggal) and YEAR(now())=YEAR(x.dari_tanggal) );

INSERT INTO `cuti_karyawan_dates`(`cuti_karyawan_id`, `tanggal_cuti`, `type`, `description`, `created_at`, `updated_at`) select a.id,cast(now() as date),1,'Leave/permit day',now(),now() From cuti_karyawan a left join cuti_karyawan_dates b on a.id = b.cuti_karyawan_id
where a.tanggal_cuti_start = cast(now() as date) and a.tanggal_cuti_end = cast(now() as date) and b.id IS NULL;
