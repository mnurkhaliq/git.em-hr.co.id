...
class UpdateEventCutiDaily5 extends Migration
{
    /**
     * Run the migrations.
     *
...
where b.join_date <= now() and (b.resign_date IS NULL or b.resign_date >= now()) and timestampdiff(day,b.join_date,NOW())>=365 and c.master_cuti_type_id=2 and DAY(b.join_date) = DAY(now()) and month(b.join_date) = month(now()) and c.iscarryforward=0;

INSERT INTO `cutilog` (`usercuti_id`, `update_status`, `date_update`) select a.id ,'Success Anniversary' ,now() From user_cuti a inner join users b on a.user_id = b.id inner join cuti c on a.cuti_id = c.id
where b.join_date <= now() and (b.resign_date IS NULL or b.resign_date >= now()) and timestampdiff(day,b.join_date,NOW())>=365 and c.master_cuti_type_id=2 and DAY(b.join_date) = DAY(now()) and month(b.join_date) = month(now()) and c.iscarryforward=0;
...
where b.join_date <= now() and (b.resign_date IS NULL or b.resign_date >= now()) and timestampdiff(day,b.join_date,NOW())>=365 and c.master_cuti_type_id=2 and DAY(b.join_date) = DAY(now()) and month(b.join_date) = month(now()) and c.iscarryforward=1;

INSERT INTO `cutilog` (`usercuti_id`, `update_status`, `date_update`) select a.id ,'Success Anniversary' ,now() From user_cuti a inner join users b on a.user_id = b.id inner join cuti c on a.cuti_id = c.id
where b.join_date <= now() and (b.resign_date IS NULL or b.resign_date >= now()) and timestampdiff(day,b.join_date,NOW())>=365 and c.master_cuti_type_id=2 and DAY(b.join_date) = DAY(now()) and month(b.join_date) = month(now()) and c.iscarryforward=1;

update user_cuti a inner join users b on a.user_id = b.id inner join cuti c on a.cuti_id = c.id set a.sisa_cuti=IF(a.kuota=0,timestampdiff(month,b.join_date,NOW())+IF(DAY(b.join_date) ='1',0,1),c.Kuota)+IF(a.sisa_cuti<c.carryforwardleave,a.sisa_cuti,c.carryforwardleave), a.cuti_terpakai=IF(a.sisa_cuti<0,ABS(a.sisa_cuti),0), a.updated_at=now(), a.kuota=IF(a.kuota=0,timestampdiff(month,b.join_date,NOW())+IF(DAY(b.join_date) ='1',0,1),c.Kuota)+IF(a.sisa_cuti<c.carryforwardleave,IF(a.sisa_cuti>0,a.sisa_cuti,0),IF(c.carryforwardleave>0,c.carryforwardleave,0))
...
INSERT INTO `cutilog` (`usercuti_id`, `update_status`, `date_update`) select a.id ,'Success Annually' ,now() From user_cuti a inner join users b on a.user_id = b.id inner join cuti c on a.cuti_id = c.id
where b.join_date <= now() and (b.resign_date IS NULL or b.resign_date >= now()) and c.master_cuti_type_id=1 and DAY(now()) ='1' and month(now())='1' and c.iscarryforward=1;

update user_cuti a inner join users b on a.user_id = b.id inner join cuti c on a.cuti_id = c.id set a.sisa_cuti=IF(a.kuota=0,timestampdiff(month,b.join_date,NOW())+IF(DAY(b.join_date) ='1',0,1),c.Kuota)+if(a.sisa_cuti<0,a.sisa_cuti,0), a.cuti_terpakai=IF(a.sisa_cuti<0,ABS(a.sisa_cuti),0), a.updated_at=now(), a.kuota=IF(a.kuota=0,timestampdiff(month,b.join_date,NOW())+IF(DAY(b.join_date) ='1',0,1),c.Kuota)
...
update user_cuti a inner join users b on a.user_id = b.id inner join cuti c on a.cuti_id = c.id set a.sisa_cuti=IF(a.kuota=0,timestampdiff(month,b.join_date,NOW())+IF(DAY(b.join_date) =REPLACE(LEFT(c.cutoffmonth, 2), \"-\", \"\"),0,1),c.Kuota)+IF(a.sisa_cuti<c.carryforwardleave,a.sisa_cuti,c.carryforwardleave), a.cuti_terpakai=IF(a.sisa_cuti<0,ABS(a.sisa_cuti),0), a.updated_at=now(), a.kuota=IF(a.kuota=0,timestampdiff(month,b.join_date,NOW())+IF(DAY(b.join_date) =REPLACE(LEFT(c.cutoffmonth, 2), \"-\", \"\"),0,1),c.Kuota)+IF(a.sisa_cuti<c.carryforwardleave,IF(a.sisa_cuti>0,a.sisa_cuti,0),IF(c.carryforwardleave>0,c.carryforwardleave,0))
...
update user_cuti a inner join users b on a.user_id = b.id inner join cuti c on a.cuti_id = c.id set a.sisa_cuti=IF(a.kuota=0,timestampdiff(month,b.join_date,NOW())+IF(DAY(b.join_date) =REPLACE(LEFT(c.cutoffmonth, 2), \"-\", \"\"),0,1),c.Kuota)+if(a.sisa_cuti<0,a.sisa_cuti,0), a.cuti_terpakai=IF(a.sisa_cuti<0,ABS(a.sisa_cuti),0), a.updated_at=now(), a.kuota=IF(a.kuota=0,timestampdiff(month,b.join_date,NOW())+IF(DAY(b.join_date) =REPLACE(LEFT(c.cutoffmonth, 2), \"-\", \"\"),0,1),c.Kuota)
...
update user_cuti a inner join users b on a.user_id = b.id inner join cuti c on a.cuti_id = c.id set a.sisa_cuti=c.Kuota+if(a.sisa_cuti<0,a.sisa_cuti,0),a.cuti_terpakai=IF(a.sisa_cuti<0,ABS(a.sisa_cuti),0),a.updated_at=now(),a.kuota=c.Kuota
where b.join_date <= now() and (b.resign_date IS NULL or b.resign_date >= now()) and timestampdiff(day,b.join_date,NOW())>=365 and c.master_cuti_type_id=3 and DAY(b.join_date) = DAY(now()) and month(b.join_date) = month(now()) and c.iscarryforward=0 and timestampdiff(year,b.join_date,NOW())<=1;
...
update user_cuti a inner join users b on a.user_id = b.id inner join cuti c on a.cuti_id = c.id set a.sisa_cuti=c.Kuota+IF(a.sisa_cuti<c.carryforwardleave,a.sisa_cuti,c.carryforwardleave), a.cuti_terpakai=IF(a.sisa_cuti<0,ABS(a.sisa_cuti),0), a.updated_at=now(), a.kuota=c.Kuota+IF(a.sisa_cuti<c.carryforwardleave,IF(a.sisa_cuti>0,a.sisa_cuti,0),IF(c.carryforwardleave>0,c.carryforwardleave,0))
where b.join_date <= now() and (b.resign_date IS NULL or b.resign_date >= now()) and timestampdiff(day,b.join_date,NOW())>=365 and c.master_cuti_type_id=3 and DAY(b.join_date) = DAY(now()) and month(b.join_date) = month(now()) and c.iscarryforward=1 and timestampdiff(year,b.join_date,NOW())<=1;

INSERT INTO `cutilog` (`usercuti_id`, `update_status`, `date_update`) select a.id ,'Success Anniversary Anniversary Annually' ,now() From user_cuti a inner join users b on a.user_id = b.id inner join cuti c on a.cuti_id = c.id
where b.join_date <= now() and (b.resign_date IS NULL or b.resign_date >= now()) and timestampdiff(day,b.join_date,NOW())>=365 and c.master_cuti_type_id=3 and DAY(b.join_date) = DAY(now()) and month(b.join_date) = month(now()) and c.iscarryforward=1 and timestampdiff(year,b.join_date,NOW())<=1;

update user_cuti a inner join users b on a.user_id = b.id inner join cuti c on a.cuti_id = c.id set a.sisa_cuti=IF(timestampdiff(year,b.join_date,NOW())<2,timestampdiff(month,b.join_date,NOW())-IF(DAY(b.join_date) ='1',12,11),c.Kuota)+IF(a.sisa_cuti<c.carryforwardleave,a.sisa_cuti,c.carryforwardleave), a.cuti_terpakai=IF(a.sisa_cuti<0,ABS(a.sisa_cuti),0), a.updated_at=now(), a.kuota=IF(timestampdiff(year,b.join_date,NOW())<2,timestampdiff(month,b.join_date,NOW())-IF(DAY(b.join_date) ='1',12,11),c.Kuota)+IF(a.sisa_cuti<c.carryforwardleave,IF(a.sisa_cuti>0,a.sisa_cuti,0),IF(c.carryforwardleave>0,c.carryforwardleave,0))
where b.join_date <= now() and (b.resign_date IS NULL or b.resign_date >= now()) and c.master_cuti_type_id=3 and DAY(now()) ='1' and month(now())='1' and c.iscarryforward=1 and timestampdiff(month,b.join_date,NOW())>12;

INSERT INTO `cutilog` (`usercuti_id`, `update_status`, `date_update`) select a.id ,'Success Annually Anniversary Annually' ,now() From user_cuti a inner join users b on a.user_id = b.id inner join cuti c on a.cuti_id = c.id
where b.join_date <= now() and (b.resign_date IS NULL or b.resign_date >= now()) and c.master_cuti_type_id=3 and DAY(now()) ='1' and month(now())='1' and c.iscarryforward=1 and timestampdiff(month,b.join_date,NOW())>12;

update user_cuti a inner join users b on a.user_id = b.id inner join cuti c on a.cuti_id = c.id set a.sisa_cuti=IF(timestampdiff(year,b.join_date,NOW())<2,timestampdiff(month,b.join_date,NOW())-IF(DAY(b.join_date) ='1',12,11),c.Kuota)+if(a.sisa_cuti<0,a.sisa_cuti,0), a.cuti_terpakai=IF(a.sisa_cuti<0,ABS(a.sisa_cuti),0), a.updated_at=now(), a.kuota=IF(timestampdiff(year,b.join_date,NOW())<2,timestampdiff(month,b.join_date,NOW())-IF(DAY(b.join_date) ='1',12,11),c.Kuota)
where b.join_date <= now() and (b.resign_date IS NULL or b.resign_date >= now()) and c.master_cuti_type_id=3 and DAY(now()) ='1' and month(now())='1' and c.iscarryforward=0 and timestampdiff(month,b.join_date,NOW())>12;
...
INSERT INTO `cutilog` (`usercuti_id`, `update_status`, `date_update`) SELECT subs.id, 'Collective Leave', Now() FROM (SELECT * FROM (SELECT a.id, a.user_id, ckd.tanggal_cuti FROM user_cuti a LEFT JOIN cuti c ON a.cuti_id = c.id LEFT JOIN users u ON a.user_id = u.id AND u.join_date <= now() AND (u.resign_date IS NULL OR u.resign_date >= now()) LEFT JOIN cuti_karyawan ck ON ck.user_id = u.id LEFT JOIN cuti_karyawan_dates ckd ON ckd.cuti_karyawan_id = ck.id AND ckd.tanggal_cuti = Cast(Now() AS date) LEFT JOIN shift s ON u.shift_id = s.id LEFT JOIN shift_detail sd ON sd.shift_id = s.id LEFT JOIN cuti_bersama z ON z.dari_tanggal = Cast(Now() AS date) WHERE c.jenis_cuti IS NOT NULL AND c.jenis_cuti = 'Annual Leave' AND (s.id IS NULL OR (s.is_collective = 0 AND sd.day IS NOT NULL AND sd.day = Dayname(Now()))) AND EXISTS (SELECT * FROM cuti_bersama x WHERE x.impacttoleave = 1 AND Day(Now()) = Day(x.dari_tanggal) AND Month(Now()) = Month(x.dari_tanggal) AND Year(Now()) = Year(x.dari_tanggal)) ORDER BY ckd.tanggal_cuti DESC LIMIT 18446744073709551615) AS sub GROUP BY sub.user_id) AS subs WHERE subs.tanggal_cuti IS NULL;
