...
class CreateEventAnnualLeaveDailyFromCutiDaily extends Migration
{
    /**
     * Run the migrations.
     *
...
CREATE DEFINER=`".config('database.connections.mysql.username')."`@`".config('database.connections.mysql.host')."` EVENT IF NOT EXISTS `Event Annual Leave Daily` ON SCHEDULE EVERY 1 DAY STARTS '2020-09-05 00:00:00' ON COMPLETION NOT PRESERVE ENABLE DO BEGIN
...
INSERT INTO ".config('database.connections.mysql.database').".event_log (`database`, `type`, `user_id`, `transaction_id`, `description`, `date`) select database(), 'Annual Leave', b.id, a.id, 'Success Monthly Cutoff Carry Forward', NOW() From user_cuti a inner join users b on a.user_id = b.id inner join cuti c on a.cuti_id = c.id
where b.join_date < DATE(NOW()) and (b.resign_date IS NULL or b.resign_date > DATE(NOW())) and c.master_cuti_type_id=4 and DAY(NOW()) =REPLACE(LEFT(c.cutoffmonth, 2), \"-\", \"\") and month(NOW())=REPLACE(RIGHT(cutoffmonth, 2), \"-\", \"\") and c.iscarryforward=1 and c.cutoffmonth is not null;
...
INSERT INTO ".config('database.connections.mysql.database').".event_log (`database`, `type`, `user_id`, `transaction_id`, `description`, `date`) select database(), 'Annual Leave', b.id, a.id, 'Success Monthly Cutoff Not Carry Forward', NOW() From user_cuti a inner join users b on a.user_id = b.id inner join cuti c on a.cuti_id = c.id
where b.join_date < DATE(NOW()) and (b.resign_date IS NULL or b.resign_date > DATE(NOW())) and c.master_cuti_type_id=4 and DAY(NOW()) =REPLACE(LEFT(c.cutoffmonth, 2), \"-\", \"\") and month(NOW())=REPLACE(RIGHT(cutoffmonth, 2), \"-\", \"\") and c.iscarryforward=0 and c.cutoffmonth is not null;
...
INSERT INTO ".config('database.connections.mysql.database').".event_log (`database`, `type`, `user_id`, `transaction_id`, `description`, `date`) select database(), 'Annual Leave', b.id, a.id, 'Success Monthly', NOW() From user_cuti a inner join users b on a.user_id = b.id inner join cuti c on a.cuti_id = c.id
where b.join_date <= DATE(NOW()) and (b.resign_date IS NULL or b.resign_date > DATE(NOW())) and c.master_cuti_type_id=4 and DAY(NOW()) ='1' and c.cutoffmonth is not null;
...
INSERT INTO ".config('database.connections.mysql.database').".event_log (`database`, `type`, `user_id`, `transaction_id`, `description`, `date`) select database(), 'Annual Leave', b.id, a.id, 'Success Anniversary Carry Forward', NOW() From user_cuti a inner join users b on a.user_id = b.id inner join cuti c on a.cuti_id = c.id
where b.join_date < DATE(NOW()) and (b.resign_date IS NULL or b.resign_date > DATE(NOW())) and c.master_cuti_type_id=2 and DAY(b.join_date) = DAY(NOW()) and month(b.join_date) = month(NOW()) and timestampdiff(year, b.join_date, DATE(NOW())) >= 1 and  c.iscarryforward=1;
...
INSERT INTO ".config('database.connections.mysql.database').".event_log (`database`, `type`, `user_id`, `transaction_id`, `description`, `date`) select database(), 'Annual Leave', b.id, a.id, 'Success Anniversary Not Carry Forward', NOW() From user_cuti a inner join users b on a.user_id = b.id inner join cuti c on a.cuti_id = c.id
where b.join_date < DATE(NOW()) and (b.resign_date IS NULL or b.resign_date > DATE(NOW())) and c.master_cuti_type_id=2 and DAY(b.join_date) = DAY(NOW()) and month(b.join_date) = month(NOW()) and timestampdiff(year, b.join_date, DATE(NOW())) >= 1 and  c.iscarryforward=0;
...
INSERT INTO ".config('database.connections.mysql.database').".event_log (`database`, `type`, `user_id`, `transaction_id`, `description`, `date`) select database(), 'Annual Leave', b.id, a.id, 'Success Annually Carry Forward', NOW() From user_cuti a inner join users b on a.user_id = b.id inner join cuti c on a.cuti_id = c.id
where b.join_date < DATE(NOW()) and (b.resign_date IS NULL or b.resign_date > DATE(NOW())) and c.master_cuti_type_id=1 and DAY(NOW()) ='1' and month(NOW())='1' and c.iscarryforward=1;
...
INSERT INTO ".config('database.connections.mysql.database').".event_log (`database`, `type`, `user_id`, `transaction_id`, `description`, `date`) select database(), 'Annual Leave', b.id, a.id, 'Success Annually Not Carry Forward', NOW() From user_cuti a inner join users b on a.user_id = b.id inner join cuti c on a.cuti_id = c.id
where b.join_date < DATE(NOW()) and (b.resign_date IS NULL or b.resign_date > DATE(NOW())) and c.master_cuti_type_id=1 and DAY(NOW()) ='1' and month(NOW())='1' and c.iscarryforward=0;
...
INSERT INTO ".config('database.connections.mysql.database').".event_log (`database`, `type`, `user_id`, `transaction_id`, `description`, `date`) select database(), 'Annual Leave', b.id, a.id, 'Success Custom Carry Forward', NOW() From user_cuti a inner join users b on a.user_id = b.id inner join cuti c on a.cuti_id = c.id
where b.join_date < DATE(NOW()) and (b.resign_date IS NULL or b.resign_date > DATE(NOW())) and c.master_cuti_type_id=5 and DAY(NOW()) =REPLACE(LEFT(c.cutoffmonth, 2), \"-\", \"\") and month(NOW())=REPLACE(RIGHT(cutoffmonth, 2), \"-\", \"\") and c.iscarryforward=1 and c.cutoffmonth is not null;
...
INSERT INTO ".config('database.connections.mysql.database').".event_log (`database`, `type`, `user_id`, `transaction_id`, `description`, `date`) select database(), 'Annual Leave', b.id, a.id, 'Success Custom Not Carry Forward', NOW() From user_cuti a inner join users b on a.user_id = b.id inner join cuti c on a.cuti_id = c.id
where b.join_date < DATE(NOW()) and (b.resign_date IS NULL or b.resign_date > DATE(NOW())) and c.master_cuti_type_id=5 and DAY(NOW()) =REPLACE(LEFT(c.cutoffmonth, 2), \"-\", \"\") and month(NOW())=REPLACE(RIGHT(cutoffmonth, 2), \"-\", \"\") and c.iscarryforward=0 and c.cutoffmonth is not null;
...
INSERT INTO ".config('database.connections.mysql.database').".event_log (`database`, `type`, `user_id`, `transaction_id`, `description`, `date`) select database(), 'Annual Leave', b.id, a.id, 'Success Anniversary Anniversary Annually', NOW() From user_cuti a inner join users b on a.user_id = b.id inner join cuti c on a.cuti_id = c.id
where b.join_date < DATE(NOW()) and (b.resign_date IS NULL or b.resign_date > DATE(NOW())) and c.master_cuti_type_id=3 and DAY(b.join_date) = DAY(NOW()) and month(b.join_date) = month(NOW()) and timestampdiff(year,b.join_date,DATE(NOW()))=1;
...
INSERT INTO ".config('database.connections.mysql.database').".event_log (`database`, `type`, `user_id`, `transaction_id`, `description`, `date`) select database(), 'Annual Leave', b.id, a.id, 'Success Annually Anniversary Annually Carry Forward', NOW() From user_cuti a inner join users b on a.user_id = b.id inner join cuti c on a.cuti_id = c.id
where b.join_date < DATE(NOW()) and (b.resign_date IS NULL or b.resign_date > DATE(NOW())) and c.master_cuti_type_id=3 and DAY(NOW()) ='1' and month(NOW())='1' and DATE_ADD(b.join_date, INTERVAL 1 YEAR) < DATE(NOW()) and c.iscarryforward=1;
...
INSERT INTO ".config('database.connections.mysql.database').".event_log (`database`, `type`, `user_id`, `transaction_id`, `description`, `date`) select database(), 'Annual Leave', b.id, a.id, 'Success Annually Anniversary Annually Not Carry Forward', NOW() From user_cuti a inner join users b on a.user_id = b.id inner join cuti c on a.cuti_id = c.id
where b.join_date < DATE(NOW()) and (b.resign_date IS NULL or b.resign_date > DATE(NOW())) and c.master_cuti_type_id=3 and DAY(NOW()) ='1' and month(NOW())='1' and DATE_ADD(b.join_date, INTERVAL 1 YEAR) < DATE(NOW()) and c.iscarryforward=0;
