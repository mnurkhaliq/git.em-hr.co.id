...
function form_validate() {
    ...
    $('.oninput input').each(function(){
        ...
        if(($(this).val() == "" || $(this).val() <= 0) && !($(this).prop('readonly')))
        {
...
$('#submit_cash_advance').click(function(){
    ...
    bootbox.confirm("Do you want process this Cash Advance?", function(result) {
        if(result)
        {
            $("#form_cash_advance").submit();
        }
    });
});
...
function select_type_(el)
{
    ...
    $.ajax({
        type: 'GET',
        url: '/karyawan/cash-advance/get-plafond',
        data: {
            'type' : $(el).val()
        },
        dataType: 'json',
