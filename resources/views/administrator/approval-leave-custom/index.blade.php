                        ...
                        <table id="data_table_no_search" class="display nowrap" cellspacing="0" width="100%">
                            ...
                            @foreach($datas as $no => $item)
                                ...
                                <tr>
                                    ...
                                    <td><a onclick="detail_approval_leave_custom({{ $item->user_id }})">
                                            {!! status_cuti($item->status) !!}
                                            </a>
                                    </td>
                                    <td>
                                        @if($item->is_approved === NULL and $item->status < 2)
                                            <a href="{{ route('administrator.approval.leave-custom.detail', $item->id) }}"> <button class="btn btn-info btn-xs m-r-5"><i class="fa fa-arrow-right"></i> process </button></a>
                                        @else
                                            <a href="{{ route('administrator.approval.leave-custom.detail', $item->id) }}"> <button class="btn btn-info btn-xs m-r-5"><i class="fa fa-search-plus"></i> detail </button></a>
                                        @endif
