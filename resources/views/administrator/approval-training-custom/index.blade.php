                            ...
                            <tbody>
                                @foreach($data as $no => $item)
                                    ...
                                    <tr>
                                        ...
                                        <td>{{ str_limit($item->topik_kegiatan, $limit = 30, $end = '...') }}</td>
                                        ...
                                        <td>
                                                ...
                                                <a href="{{ route('administrator.approval.training-custom.detail', $item->id) }}" class="btn btn-info btn-xs"> <i class="fa fa-arrow-right"></i> process</a>
                                                @else
                                                <a href="{{ route('administrator.approval.training-custom.detail', $item->id) }}" class="btn btn-info btn-xs"> <i class="fa fa-search-plus"></i> detail</a>
                                                @endif
                                        </td>
                                        <td>
                                            @if($item->status == 2)
                                                ...
                                                @if(cek_level_training_up($item->training->id))
                                                    ...
                                                    <a href="{{ route('administrator.approval.training-custom.claim', $item->id) }}" class="btn btn-info btn-xs"> <i class="fa fa-arrow-right"></i> process claim</a>
                                                @endif
                                                @elseif($item->is_approved_claim != NULL and $item->status_actual_bill >= 1)
                                                    <a href="{{ route('administrator.approval.training-custom.claim', $item->id) }}" class="btn btn-info btn-xs"> <i class="fa fa-search-plus"></i> claimed detail</a>
                                                @endif
                                            @endif
                                        </td>
                                    </tr>
