                    ...
                    <div class="white-box">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-12">
                                ...
                                @if(!empty($data->thumbnail) && file_exists(public_path('storage/internal-memo/').$data->thumbnail))
                                    <div><img src="{{ asset('storage/internal-memo/'. $data->thumbnail) }}" /></div>
                                @endif
                                </output>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-12">
                                ...
                                @if(!empty($data->image) && file_exists(public_path('storage/internal-memo/').$data->image))
                                    <div><img src="{{ asset('storage/internal-memo/'. $data->image) }}" /></div>
                                @endif
                                </output>
    ...
    <script type="text/javascript">
        ...
        function showAttachment(){
            ...
            @foreach($data->files as $file)
                @if(!empty($file->file) && file_exists(public_path('storage/internal-memo/').$file->file))
                    img = "{{ asset('storage/internal-memo/'. $file->file) }}";
                    var ext = img.split('.').pop().toLowerCase();
                    if(ext === 'pdf'){
                        div += '<div><embed src="'+img+'" ></div>';
                    } else {
                        div += '<div><img src="'+img+'" /></div>';
                    }
                @endif
            @endforeach
            $('#default_file').html(div)
        }
