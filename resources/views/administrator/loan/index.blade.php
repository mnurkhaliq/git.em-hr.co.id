        ...
        <div class="row bg-title" style="overflow: inherit;">
            ...
            <div class="col-lg-9 col-sm-8 col-md-8 col-xs-12">
                <div class="col-md-12 pull-right" style="padding:0px;">
                    <form method="POST" action="{{ route('administrator.loan.index') }}" id="filter-form">
                        {{ csrf_field() }}
                            ...
                            <div style="padding-left:0; float: right;">
                                <div class="btn-group pull-right">
                                    ...
                                    <ul role="menu" class="dropdown-menu" style="min-width: 10px; color: blue;">
                                        ...
                                        <li><a class="toggle-vis" data-column="3" style="color:blue;">Posititon</a></li>
                                        <li><a class="toggle-vis" data-column="4" style="color:blue;">Request Date</a></li>
                                        <li><a class="toggle-vis" data-column="5" style="color:blue;">Calculated Amount (IDR)</a></li>
                            ...
                            <div class="form-group  m-b-0">
                                <select class="form-control form-control-line" name="division_id">
                                    <option value=""> - Choose Division - </option>
                                        ...
                                        <option value="{{ $item->id }}" {{ $item->id== request()->division_id || Session::get('loan-division_id') == $item->id ? 'selected' : '' }}>{{ $item->name }}</option>
                                        @endforeach
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="form-group  m-b-0">
                                <select class="form-control form-control-line" name="position_id">
                                    <option value=""> - Choose Position - </option>
                                        ...
                                        <option value="{{ $item->id }}" {{ $item->id== request()->position_id || Session::get('loan-position_id') == $item->id ? 'selected' : '' }}>{{ $item->name }}</option>
                                        @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="col-md-3 pull-right">
                            <div class="form-group m-b-0">
                                <select class="form-control form-control-line" name="employee_status">
                                    ...
                                    <option {{ (request() and request()->employee_status == 'Permanent') || Session::get('loan-employee_status') == 'Permanent' ? 'selected' : '' }}>Permanent</option>
                                    <option {{ (request() and request()->employee_status == 'Contract') || Session::get('loan-employee_status') == 'Contract' ? 'selected' : '' }}>Contract</option>
                                    <option {{ (request() and request()->employee_status == 'Internship') || Session::get('loan-employee_status') == 'Internship' ? 'selected' : '' }}>Internship</option>
                                    <option {{ (request() and request()->employee_status == 'Outsource') || Session::get('loan-employee_status') == 'Outsource' ? 'selected' : '' }}>Outsource</option>
                                    <option {{ (request() and request()->employee_status == 'Freelance') || Session::get('loan-employee_status') == 'Freelance' ? 'selected' : '' }}>Freelance</option>
                                    <option {{ (request() and request()->employee_status == 'Consultant') || Session::get('loan-employee_status') == 'Consultant' ? 'selected' : '' }}>Consultant</option>
                                </select>
                            </div>
                        </div>
                        <div class="col-md-3 pull-right">
                            <div class="form-group  m-b-0">
                                <input type="text" name="name" class="form-control form-control-line" value="{{ (request() and request()->name) || Session::get('loan-name') ? Session::get('loan-name') : '' }}" placeholder="Name / NIK Employee">
                            </div>
                        </div>
                            ...
                            <tbody>
                                @foreach($data as $no => $item)
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <a onclick="detail_approval_loan_custom({{ $item->id }})"> 
                                            {!! status_loan($item->status) !!}
                                            </a>
                                        </td>
                                        <td>
                                            @if(!$item->historyApproval->where('is_approved', '!=', 1)->count() && $item->status == 1)
                                                <a href="{{ route('administrator.loan.proses', $item->id) }}"> <button class="btn btn-info btn-xs m-r-5"><i class="fa fa-arrow-right"></i> process</button></a>
                                            @else
                                                <a href="{{ route('administrator.loan.proses', $item->id) }}"> <button class="btn btn-info btn-xs m-r-5"><i class="fa fa-search-plus"></i> detail</button></a>
                                            @endif
                                        </td>
                                    </tr>
                                @endforeach
