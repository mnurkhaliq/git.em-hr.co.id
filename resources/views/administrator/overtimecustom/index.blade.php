                            ...
                            <div style="padding-left:0; float: right;">
                                <div class="btn-group pull-right">
                                    ...
                                    <ul role="menu" class="dropdown-menu" style="min-width: 10px; color: blue;">
                                        ...
                                        <li><a class="toggle-vis" data-column="4" style="color:blue;">Overtime Submission </a></li>
                                        <li><a class="toggle-vis" data-column="5" style="color:blue;">Submission Status</a></li>
                                        <li><a class="toggle-vis" data-column="6" style="color:blue;">Claim Status</a></li> 
                        ...
                        <div class="col-md-2 pull-right">
                            <div class="form-group  m-b-0">
                                <select class="form-control form-control-line" name="division_id">
                                        ...
                                        <option value="{{ $item->id }}" {{ $item->id== request()->division_id || Session::get('o-division_id') == $item->id ? 'selected' : '' }}>{{ $item->name }}</option>
                                        @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="col-md-2 pull-right">
                            <div class="form-group  m-b-0">
                                <select class="form-control form-control-line" name="position_id">
                                        ...
                                        <option value="{{ $item->id }}" {{ $item->id== request()->position_id || Session::get('o-position_id') == $item->id ? 'selected' : '' }}>{{ $item->name }}</option>
                                        @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="col-md-3 pull-right">
                            <div class="form-group  m-b-0">
                                ...
                                <option value="">- Employee Status - </option>
                                    <option {{ (request() and request()->employee_status == 'Permanent') || Session::get('o-employee_status') == 'Permanent' ? 'selected' : '' }}>Permanent</option>
                                    <option {{ (request() and request()->employee_status == 'Contract') || Session::get('o-employee_status') == 'Contract' ? 'selected' : '' }}>Contract</option>
                                    <option {{ (request() and request()->employee_status == 'Internship') || Session::get('o-employee_status') == 'Internship' ? 'selected' : '' }}>Internship</option>
                                    <option {{ (request() and request()->employee_status == 'Outsource') || Session::get('o-employee_status') == 'Outsource' ? 'selected' : '' }}>Outsource</option>
                                    <option {{ (request() and request()->employee_status == 'Freelance') || Session::get('o-employee_status') == 'Freelance' ? 'selected' : '' }}>Freelance</option>
                                    <option {{ (request() and request()->employee_status == 'Consultant') || Session::get('o-employee_status') == 'Consultant' ? 'selected' : '' }}>Consultant</option>
                                </select>
                            </div>
                        </div>
                        <div class="col-md-3 pull-right">
                            <div class="form-group  m-b-0">
                                <input type="text" name="name" class="form-control form-control-line" value="{{ (request() and request()->name) || Session::get('o-name') ? Session::get('o-name') : '' }}" placeholder="Name / NIK Employee">
                            </div>
                        </div>
                            ...
                            <tbody>
                                @foreach($data as $no => $item)
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <a href="{{ route('administrator.overtimeCustom.proses', $item->id) }}" class="btn btn-info btn-xs"><i class="fa fa-search-plus"></i> detail </a>
                                            @if($item->status == 2 and $item->status_claim >= 1) 
                                                <a href="{{ route('administrator.overtimeCustom.claim', $item->id) }}" class="btn btn-info btn-xs"> <i class="fa fa-search-plus"></i> claimed detail </a>
                                            @endif
                                        </td>
                                    </tr>
                                @endforeach
