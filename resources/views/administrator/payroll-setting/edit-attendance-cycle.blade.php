...
@section('title', 'Form Setting Attendance Cycle')
    ...
    <div class="container-fluid">
        <div class="row bg-title">
            <div class="col-lg-3 col-md-4 col-sm-4 col-xs-12">
                <h4 class="page-title">Form Setting Attendance Cycle</h4> </div>
            <div class="col-lg-9 col-sm-8 col-md-8 col-xs-12">

                <ol class="breadcrumb">
                    <li><a href="javascript:void(0)">Dashboard</a></li>
                    <li class="active">Setting Attendance Cycle</li>
                </ol>
            </div>
            <!-- /.col-lg-12 -->
        </div>
        <!-- .row -->
        <div class="row">
            <form class="form-horizontal" enctype="multipart/form-data" action="{{ route('administrator.payroll-setting.update-attendance-cycle', $data->id) }}" method="POST">
                <div class="col-md-12">
                    <div class="white-box">
                        <h3 class="box-title m-b-0">Edit Setting Attendance Cycle</h3>
                        <hr />
                        @if (count($errors) > 0)
                        ...
                        {{ csrf_field() }}
                        <input type="hidden" name="key_name" value="attendance_custom" />
                        <div class="form-group">
                            <label class="col-md-12">Label</label>
                            <div class="col-md-6">
                               <input type="text" name="label" class="form-control" value="{{ $data->label }}" required>
                            </div>
                        </div>
                        <div class="form-group">
                            ...
                            <div class="col-md-6">
                                <select id="start_date" class="form-control" disabled>
                                    ...
                                    @for($i=1; $i<=31; $i++)
                                        <option value="{{$i}}" {{ $data->start_date == $i ? 'selected' : '' }}>{{$i}}</option>
                                    @endfor
                                </select>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-6">
                                <select id="end_date" name="end_date" class="form-control" required>
                                    ...
                                    @for($i=1; $i<=31; $i++)
                                        <option value="{{$i}}" {{ $data->end_date == $i ? 'selected' : '' }}>{{$i}}</option>
                                    @endfor
                                </select>
                            </div>
                        </div>
            ...
            <form id="cancel" action="{{ route('administrator.payroll-setting.index') }}" method="GET">
                <input type="hidden" name="tab" value="cycleAttendance" />
            </form>                  
        </div>
        <!-- /.row -->
        <!-- ============================================================== -->
