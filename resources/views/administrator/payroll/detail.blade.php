               ...
               <div class="col-md-4 p-l-0 source">
                    <div class="white-box" style="min-height: 440px;">
                        ...
                        <table class="table table-stripped m-t-20">
                            ...
                            @endif
                            @if($payroll = get_user_payroll_cycle('payroll', $data->user_id))
                            <tr>
                                <th>Payroll Cycle</th>
                                <th>:</th>
                                <th class="td-payroll_cycle">{{ ($payroll->label ?: 'Default') . ' (' . $payroll->start_date . ' - ' . $payroll->end_date . ')' }}</th>
                            </tr>
                            @endif
                            @if($attendance = get_user_payroll_cycle('attendance', $data->user_id))
                            <tr>
                                <th>Attendance Cycle</th>
                                <th>:</th>
                                <th class="td-attendance_cycle">{{ ($attendance->label ?: 'Default') . ' (' . $attendance->start_date . ' - ' . $attendance->end_date . ')' }}</th>
                            </tr>
                            @endif
                            @if(!empty($month) && !empty($year))
                            <tr>
        ...
        function getAttendanceAndOvertime() {
            ...
            if(user_id==''){
                ...
                $.ajax({
                    ...
                    data: {'user_id' : user_id,'month':"{{$month}}",'year':'{{$year}}', '_token' : $("meta[name='csrf-token']").attr('content')},
                    dataType: 'json',
                    success: function (data) {
