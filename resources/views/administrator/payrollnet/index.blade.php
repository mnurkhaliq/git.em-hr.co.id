    ...
    <div class="container-fluid">
        <div class="row bg-title">
            ...
            <div class="col-lg-9 col-sm-8 col-md-8 col-xs-12">
                ...
                <a href="{{ route('administrator.payrollnet.download') }}" class="btn btn-success btn-sm pull-right m-l-20 hidden-xs hidden-sm waves-effect waves-light"> <i class="fa fa-cloud-download"></i> DOWNLOAD PAYROLL</a>
                <a href="{{ route('administrator.payrollnet.create') }}" class="btn btn-success btn-sm pull-right m-l-20 hidden-xs hidden-sm waves-effect waves-light"> <i class="fa fa-plus"></i> CREATE PAYROLL</a>
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="white-box">
                    ...
                    <form method="POST" action="{{ route('administrator.payrollnet.index') }}" id="filter-form">
                        <p>Filter Form</p>
                        {{ csrf_field() }}
                        ...
                        <div class="col-md-2" style="padding-left:0;">
                            <div class="form-group">
                                <select class="form-control" name="employee_status">
                                    ...
                                    <option {{ (request() and request()->employee_status == 'Internship') ? 'selected' : '' }}>Internship</option>
                                    <option {{ (request() and request()->employee_status == 'Outsource') ? 'selected' : '' }}>Outsource</option>
                                    <option {{ (request() and request()->employee_status == 'Freelance') ? 'selected' : '' }}>Freelance</option>
                                </select>
                            </div>
                        </div>
	                            ...
	                            @foreach($data as $no => $item)
	                            	@if(isset($item->user))
			                            <tr>
			                                ...
			                                <td>
			                                    <a href="{{ route('administrator.payrollnet.detail', $item->id) }}" class="btn btn-info btn-xs"><i class="fa fa-edit"></i> detail</a>
			                                </td>
			                            </tr>
			                            @php ($i ++)
                                    @else
        ...
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                    ...
                    <form method="POST" id="form-upload" enctype="multipart/form-data" class="form-horizontal frm-modal-education" action="{{ route('administrator.payrollnet.temp-import') }}">
                    {{ csrf_field() }}
                    <div class="modal-body">
                        ...
                        <a href="{{ asset('storage/sample/Payrollnet.xlsx') }}"><i class="fa fa-download"></i> Download Sample Excel</a>
                    </div>
...
<script type="text/javascript">
    $("#calculate").click(function(){
        ...
        bootbox.confirm('Calculate Payroll ?', function(res){
            ...
            setTimeout(function(){
                window.location = '{{ route('administrator.payrollnet.calculate') }}';
            }, 2000);
        });
