...
@section('content')
    ...
    <div id="page-wrapper">
        <div class="container-fluid">
            ...
            <!-- .row -->
            <div class="row">
                <form id="form" class="form-horizontal" enctype="multipart/form-data" action="{{ route('administrator.remote-attendance.update',$data->id) }}" method="POST">
                    @method('PUT')
                    <div class="col-md-12">
                        <div class="white-box">
                            <h3 class="box-title m-b-0">Edit Remote Attendance</h3>
                            <hr />
                            @if (count($errors) > 0)
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-md-6">
                                    <input type="text" name="user" class="form-control"  value="{{ $data->user->nik.' - '.$data->user->name }}" disabled>
                                </div>
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-md-3">
                                    <input type="text" name="start_date" class="form-control" id="from" placeholder="Start Date" value="{{ old('start_date',date('m/d/Y',strtotime($data->start_date)))}}"/>
                                </div>
                                <div class="col-md-3">
                                    <input type="text" name="end_date" class="form-control" id="to" placeholder="End Date" value="{{ old('end_date',date('m/d/Y',strtotime($data->end_date)))}}">
                                </div>
                            </div>
                            <div class="col-md-6"  style="padding: 0">
                                <div class="form-group col-md-6"  style="padding: 0">
                                    ...
                                    <div class="col-md-12">
                                        <input id="latitude" name="latitude" type="number" step="any" class="form-control" value="{{ old('latitude',$data->latitude) }}">
                                    </div>
                                </div>
                                <div class="form-group col-md-6"  style="padding-right: 0">
                                    <label class="col-md-12">Longitude</label>
                                    <div class="col-md-12">
                                        <input id="longitude" name="longitude" type="number" step="any" class="form-control" value="{{ old('longitude',$data->longitude) }}">
                                    </div>
                                </div>
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-md-6">
                                    <input type="number" id="radius" name="radius" class="form-control" value="{{ old('radius',$data->radius) }}">
                                </div>
                            </div>
...
@section('js')
    <script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyBTctq_RFrwKOd84ZbvJYvU3MEcrLmPNJ8"
            async defer></script>
    <script>
        $('#timezone').val("{{old('timezone',$data->timezone)}}");
        $( "#from, #to" ).datepicker({
            defaultDate: "+1w",
            changeMonth: true,
            numberOfMonths: 2
        ...
        setTimeout(function() {
            ...
            @elseif($data->latitude != null && $data->latitude != 0)
            cityCircle.setCenter({lat: {{$data->latitude}}, lng: {{$data->longitude}}});
            marker.setPosition({lat: {{$data->latitude}}, lng: {{$data->longitude}}});
            map.setCenter({lat: {{$data->latitude}}, lng: {{$data->longitude}}});
            cityCircle.setRadius({{$data->radius}});
            map.setZoom(16);
