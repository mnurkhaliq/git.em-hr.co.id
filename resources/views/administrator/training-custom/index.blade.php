    ...
    <div class="container-fluid">
        <div class="row bg-title">
            ...
            <div class="col-lg-9 col-sm-8 col-md-8 col-xs-12">
                <a href="{{ route('administrator.training-custom.create') }}" class="btn btn-success btn-sm pull-right m-l-20  widthaves-effect waves-light"> <i class="fa fa-plus"></i> ADD BUSINESS TRIP</a>
                            ...
                            <tbody>
                                @foreach($data as $no => $item)
                                    <tr>
                                        ...
                                        <td>
                                            <a href="{{ route('administrator.training-custom.edit', $item->id) }}" class="btn btn-info btn-xs"><i class="fa fa-search-plus"></i> detail</a>
                                        </td>
                                        <td>
                                            @if($item->status == 2)
                                                @if(empty($item->status_actual_bill) or $item->status_actual_bill < 1)
                                                    <a href="{{ route('administrator.training-custom.claim', $item->id) }}" class="btn btn-info btn-xs"><i class="fa fa-book"></i> claim</a>
                                                @else
                                                    @if($item->status_actual_bill >= 1)
                                                    <a href="{{ route('administrator.training-custom.claim', $item->id) }}">
