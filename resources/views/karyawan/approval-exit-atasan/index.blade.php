                                ...
                                @foreach($data as $no => $item)
                                  @if(isset($item->user->nik))
                                    <tr>
                                        ...
                                        <td>
                                           @if($item->is_approved_atasan == NULL)
                                                <a href="{{ route('karyawan.approval.exit-atasan.detail', $item->id) }}"> <button class="btn btn-info btn-xs m-r-5"><i class="fa fa-arrow-right"></i> proses</button></a>
                                            @else 
                                                <a href="{{ route('karyawan.approval.exit-atasan.detail', $item->id) }}"> <button class="btn btn-info btn-xs m-r-5"><i class="fa fa-search-plus"></i> detail</button></a>
                                            @endif
                                        </td>
                                    </tr>
                                  @endif
