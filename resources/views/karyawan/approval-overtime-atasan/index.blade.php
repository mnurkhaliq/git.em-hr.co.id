                            ...
                            <tbody>
                                @foreach($data as $no => $item)
                                    <tr>
                                        ...
                                        <td>
                                            @if($item->is_approved_atasan == 0)
                                                <a href="{{ route('karyawan.approval.overtime-atasan.detail', $item->id) }}"> <button class="btn btn-info btn-xs m-r-5">proses <i class="fa fa-arrow-right"></i></button></a>
                                            @else
                                                <a href="{{ route('karyawan.approval.overtime-atasan.detail', $item->id) }}"> <button class="btn btn-info btn-xs m-r-5"><i class="fa fa-search-plus"></i> detail</button></a>
                                            @endif
                                        </td>
                                    </tr>
                                @endforeach
