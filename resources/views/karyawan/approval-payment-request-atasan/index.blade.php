...
-->
                                            ...
                                            @elseif($item->status ==4)
                                                 <label class="btn btn-default btn-xs" onclick="bootbox.alert('<h4>Reason</h4><hr /><p>{{ $item->note_pembatalan }}</p>')">Cancelled</label>
                                            @endif
                                        ...
                                        <td>
                                            @if(!cek_status_approval_user(Auth::user()->id, 'payment_request', $item->id)  and $item->status < 4)
                                                <a href="{{ route('karyawan.approval.payment-request-atasan.detail', $item->id) }}"> <button class="btn btn-info btn-xs m-r-5">proses <i class="fa fa-arrow-right"></i></button></a>
                                            @else
                                                <a href="{{ route('karyawan.approval.payment-request-atasan.detail', $item->id) }}"> <button class="btn btn-default btn-xs m-r-5">detail <i class="fa fa-search-plus"></i></button></a>
                                            @endif
                                        </td>
