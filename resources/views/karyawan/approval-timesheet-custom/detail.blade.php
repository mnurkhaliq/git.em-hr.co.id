                ...
                <div class="col-md-12">
                    <div class="white-box">
                            ...
                            <table class="table table-hover manage-u-table">
                                ...
                                <tbody class="table-content-lembur">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="radio reject icheck-danger col-xs-6 p-0">
                                                <input class="approval" type="radio" id="danger-{{ $no }}" data-index="{{ $no }}" name="transactions[{{ $no }}][is_approved]" {{ $item->status != 1 || $item->timesheetCategory->settingApproval->count() < 1 || !$item->timesheetCategory->settingApproval->where('user_id', \Auth::user()->id)->first() ? 'disabled' : '' }} {{ (isset(old('transactions')[$no]['is_approved']) && old('transactions')[$no]['is_approved'] === "0") || $item->status == 3 ? 'checked' : '' }} value="0">
                                                <label for="danger-{{ $no }}"></label>
                                            </div>
                                            <div class="radio icheck-success col-xs-6 p-0 text-right">
                                                <input class="approval" type="radio" id="success-{{ $no }}" data-index="{{ $no }}" name="transactions[{{ $no }}][is_approved]" {{ $item->status != 1 || $item->timesheetCategory->settingApproval->count() < 1 || !$item->timesheetCategory->settingApproval->where('user_id', \Auth::user()->id)->first() ? 'disabled' : '' }} {{ (isset(old('transactions')[$no]['is_approved']) && old('transactions')[$no]['is_approved'] === "1") || $item->status == 2 ? 'checked' : '' }} value="1">
                                                <label for="success-{{ $no }}"></label>
                                            </div>
                                            <input type="hidden" id="approving-{{ $no }}" name="transactions[{{ $no }}][is_approving]" value="{{ isset(old('transactions')[$no]['is_approving']) ? old('transactions')[$no]['is_approving'] : 0 }}" />
                                        </td>
                                        <td></td>
                                        @if($item->status == 3)
                                        ...
                                        @elseif($item->status == 1 && $item->timesheetCategory->settingApproval->count() > 0 && $item->timesheetCategory->settingApproval->where('user_id', \Auth::user()->id)->first())
                                            <td style="background-color: #ffa82b;"></td>
                                        @else
                                            <td style="background-color: #e4e7ea;"></td>
                                        @endif
                                    </tr>
                                    @if($item->approval_note && !isset(old('transactions')[$no]['is_approved']))
                                    <tr class="approval-note" id="approval-note-{{ $no }}">
                                        <td></td>
                                        <td>APPROVAL NOTE</td>
                                        <td colspan="7">
                                            <input type="text" name="transactions[{{ $no }}][note]" {{ $item->status != 1 ? 'readonly' : '' }} class="form-control" value="{{ $item->approval_note }}" />
                                        </td>
                                        <td colspan="2"></td>
                                        @if($item->status == 3)
                                            <td style="background-color: #ff7676;"></td>
                                    ...
                                    <tr class="approval-note" id="approval-note-{{ $no }}" style="display: {{ !isset(old('transactions')[$no]['is_approved']) || (isset(old('transactions')[$no]['is_approved']) && old('transactions')[$no]['is_approved'] === "1") ? 'none' : '' }}">
                                        ...
                                        <td>APPROVAL NOTE</td>
                                        <td colspan="7">
                                            <input type="text" name="transactions[{{ $no }}][note]" class="form-control" value="{{ isset(old('transactions')[$no]['note']) ? old('transactions')[$no]['note'] : '' }}" />
                                        </td>
                                        <td colspan="2"></td>
                                        @if($item->status == 3)
                                            <td style="background-color: #ff7676;"></td>
                        ...
                        <div class="form-group">                            
                            ...
                            @php($is_approvable = $data->where('id', $data->id)->whereHas('timesheetPeriodTransaction', function ($query) {
                            ...
                            @if($is_approvable)
                            <a class="btn btn-sm btn-success waves-effect waves-light m-r-10" id="btn_submit"><i class="fa fa-check"></i> Submit</a>
                            @endif
...
<script type="text/javascript">
    $(".approval").change(function() {
        ...
        $('#approving-'+$(this).data('index')).val(1)
    });
