                                ...
                                @foreach($data as $no => $item)
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            @if($item->status_actual_bill == 2 and $item->is_approve_atasan_actual_bill == 0)
                                             <a href="{{ route('karyawan.approval.training-atasan.biaya', $item->id) }}"> <button class="btn btn-info btn-xs m-r-5"><i class="fa fa-file"></i> Proses Actual Bill</button></a>
                                            @endif

                                            @if($item->is_approve_atasan_actual_bill == 1 || $item->is_approve_atasan_actual_bill === 0 )
                                             <a href="{{ route('karyawan.approval.training-atasan.biaya', $item->id) }}"> <button class="btn btn-info btn-xs m-r-5"><i class="fa fa-file"></i> Detail Actual Bill</button></a>
                                            @endif

                                            @if($item->is_approved_atasan == 0)
                                                <a href="{{ route('karyawan.approval.training-atasan.detail', $item->id) }}"> <button class="btn btn-info btn-xs m-r-5">proses <i class="fa fa fa-arrow-right"></i></button></a>
                                            @else
                                                <a href="{{ route('karyawan.approval.training-atasan.detail', $item->id) }}"> <button class="btn btn-info btn-xs m-r-5">detai <i class="fa fa-search-plus"></i></button></a>
                                            @endif
                                        </td>
                                    </tr>
                                @endforeach
