            ...
            <form class="form-horizontal" id="form-actual-bill" enctype="multipart/form-data" action="{{ route('karyawan.approval.training-custom.prosesClaim') }}" method="POST">
                <div class="col-md-12">
                    <div class="white-box">
                        ...
                        elseif($data->status_actual_bill < 2 && $history->is_approved_claim != NULL ){
                            $readonly = ' readonly="true"'; 
                        }
                        ?>
                                ...
                                <tbody class="table-content-value">
                                    @foreach($data->training_allowance as $no => $item2)
                                        <tr>
                                             ...
                                             <td>
                                                <input type="text" name="morning_approved[]"  class="form-control price_format morning_approved" value="{{ $item2->morning_approved != null || $item2->morning_approved == 0 ? format_idr($item2->morning_approved) :  format_idr($item2->morning) }}" {{$readonly}}>
                                            </td>
                                            ...
                                            <td>
                                                <input type="text" name="afternoon_approved[]"  class="form-control price_format afternoon_approved" value="{{ $item2->afternoon_approved != null || $item2->afternoon_approved == 0 ? format_idr($item2->afternoon_approved) :  format_idr($item2->afternoon) }}" {{$readonly}}>
                                            </td>
                                            <td><input type="text" class="form-control evening price_format" readonly="true" name="evening[]" value="{{ format_idr($item2->evening) }}"></td>
                                            <td class="spacer2">
                                                <input type="text" name="evening_approved[]"  class="form-control price_format evening_approved" value="{{ $item2->evening_approved != null || $item2->evening_approved == 0 ? format_idr($item2->evening_approved) :  format_idr($item2->evening) }}" {{$readonly}}>
                                        ...
                                        <tr>
                                            <input type="hidden" name="id_daily[]" class="form-control"  value="{{ $item3->id }}" readonly="true">
                                             ...
                                             <td class="spacer">
                                                ...
                                                <input type="text" name="nominalDaily_approved[]"  class="form-control price_format nominalDaily_approved" value="{{ format_idr($item3->daily_approved) }}" {{$readonly}}>
                                                @if(count($item3->report) > 0)
                                                <label onclick="show_history('{{$item3->id}}', 'daily')" class="btn btn-info btn-xs">history</label>
                                                @endif
                                                @endif
                                                 @if($item3->daily_approved == null)
                                                <input type="text" name="nominalDaily_approved[]" class="form-control price_format nominalDaily_approved" value="{{ format_idr($item3->daily) }}" {{$readonly}}>
                                                @endif
                                            </td>
                                            <td title="{{$item3->note}}"><input type="text" readonly="true" class="form-control noteDaily" name="noteDaily[]" value="{{ $item3->note}}"></td>
                                            <td title="{{$item3->note_approval}}">
                                        ...
                                        <tr>
                                            <input type="hidden" name="id_other[]" class="form-control"  value="{{ $item4->id }}" readonly="true">
                                             ...
                                             <td class="spacer">
                                                ...
                                                <input type="text" name="nominalOther_approved[]"  class="form-control price_format nominalOther_approved" value="{{ format_idr($item4->nominal_approved) }}" {{$readonly}}>
                                                @if(count($item4->report) > 0)
                                                <label onclick="show_history('{{$item4->id}}', 'other')" class="btn btn-info btn-xs">history</label>
                                                @endif
                                                @endif
                                                @if($item4->nominal_approved == null)
                                                <input type="text" name="nominalOther_approved[]" class="form-control price_format nominalOther_approved" value="{{ format_idr($item4->nominal) }}" {{$readonly}}>
                                                @endif
                                            </td>
                                             <td title="{{$item4->note}}"><input type="text" readonly="true" class="form-control noteOther" name="noteOther[]" value="{{ $item4->note}}"></td>
                                             <td title="{{$item4->note_approval}}">
                                                <input type="text" name="noteOther_approved[]"  class="form-control noteOther_approved" value="{{$item4->note_approval != null ? $item4->note_approval : ''}}" {{$readonly}}>
                                            </td>
                                            <td>
                    ...
                    <br />
                       ...
                       <div class="form-group">
                            ...
                            @else
                            <div class="col-md-12">
                                 <textarea class="form-control note_claim" id="noteApproval" name="note_claim" placeholder="Note Claim Approval ">{{ $history->note_claim }}</textarea>
                            </div>
                             @endif
                        </div>
...
$(document).ready(function () {
    if('{{$data->status_actual_bill < 2 && $history->is_approved_claim == NULL}}'){
 ...
 var calculate_nominalAcomodation  = function(){
    ...
    $('.sub_total_1_disetujui').html(numberWithComma(totalnominalAcomodation).replace(/,/g, "."));
...
var calculate_allAllowance  = function(){
        ...
        $('.sub_total_2_disetujui').html(numberWithComma(totalAll).replace(/,/g, "."));
...
var calculate_nominalDaily  = function(){
    ...
    $('.sub_total_3_disetujui').html(numberWithComma(totalnominalDaily).replace(/,/g, "."));
...
var calculate_nominalOther  = function(){
    ...
    $('.sub_total_4_disetujui').html(numberWithComma(totalnominalOther).replace(/,/g, "."));
...
$(".nominalOther_approved").on('input', function(){
    ...
    function calculate_all()
    {
        ...
        $('.total_actual_bill_disetujui').html(numberWithComma(total_actual).replace(/,/g, "."));
        $('.total_reimbursement_disetujui').html(numberWithComma(total_reimbursement).replace(/,/g, "."));
    }
    ...
    $("#btn_approved").click(function(){
        if(!$("#noteApproval").val()){
            ...
            bootbox.confirm('Approve Actual Bill Business Trip ?', function(result){
    ...
    $("#btn_tolak").click(function(){
        if(!$("#noteApproval").val()){
            ...
            $("#noteApproval").removeClass('noteError');
            bootbox.confirm('Reject Actual Bill Business Trip ?', function(result){
                if(result)
                {
                    $('#form-actual-bill').submit();
                }
