                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @if($item->status ==2)
                                                @if($item->status_actual_bill >= 2)
                                                    ...
                                                    @if($item->is_approve_atasan_actual_bill == 1)
                                                        ...
                                                        @if($item->approve_direktur_actual_bill === NULL)
                                                         <a href="{{ route('karyawan.approval.training.biaya', $item->id) }}"> <button class="btn btn-info btn-xs m-r-5"><i class="fa fa-file"></i> Proses Actual Bill</button></a>
                                                        @endif

                                                        @if($item->approve_direktur_actual_bill == 1 || $item->approve_direktur_actual_bill === 0)
                                                            <a href="{{ route('karyawan.approval.training.biaya', $item->id) }}"> <button class="btn btn-info btn-xs m-r-5"><i class="fa fa-file"></i> Detail Actual Bill</button></a>
                                                        @endif
                                                    @endif
                                                @endif
                                            @endif
                                            ...
                                            @if($item->approve_direktur == 0)
                                                <a href="{{ route('karyawan.approval.training.detail', $item->id) }}"> <button class="btn btn-info btn-xs m-r-5">Proses <i class="fa fa fa-arrow-right"></i></button></a>
                                            @else
                                                <a href="{{ route('karyawan.approval.training.detail', $item->id) }}"> <button class="btn btn-info btn-xs m-r-5">Detail <i class="fa fa-search-plus"></i></button></a>
                                            @endif
                                        </td>
                                    </tr>
                                @endforeach
