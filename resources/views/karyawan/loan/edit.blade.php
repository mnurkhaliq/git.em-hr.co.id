                ...
                <div class="white-box">
                    ...
                    <div class="tab-content">
                        <div role="tabpanel" class="tab-pane fade active in" id="Form">
                            <h3>Form Loan</h3>
                            <br />
                            <form class="form-horizontal" enctype="multipart/form-data" action="{{ route('karyawan.loan.update', $data->id) }}" method="POST">
                                ...
                                <br />
                                <div class="col-md-12">
                                    <a href="{{ route('karyawan.loan.index') }}" class="btn btn-sm btn-default waves-effect waves-light m-r-10"><i class="fa fa-arrow-left"></i> Back</a>
                                    <br style="clear: both;" />
                                </div>
                                <div class="clearfix"></div>
                            </form>  
...
<script>
    ...
    function initMyTable() {
        ...
        myTable = $('#myTable').DataTable({
            ajax: {
                "url": "{{ route('karyawan.loan.table', $data->id) }}",
                "type": "GET"
            },
            searching: false,
            paging: false,
    ...
    window.onload = function() {
        ...
        if (window.File && window.FileList && window.FileReader) {
            ...
            $("#photo").on("change", function(event) {
                var files = event.target.files; //FileList object
                var output = document.getElementById("result_photo");
                $("#result_photo").html('');
                if (!files.length) {
                    $("#result_photo").html(result);
                }
    ...
    $("#btn_submit").click(function(){
        if($(".asset").css('display') != 'none') {
            ...
            if (error) {
                bootbox.alert("Loan collateral data is incomplete !");
                return false;
            }
        }
