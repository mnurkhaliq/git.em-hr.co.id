                            ...
                            <table class="table table-hover manage-u-table">
                                ...
                                <tbody class="table-content-timesheet">
                                    ...
                                    <tr>
                                        ...
                                        @if($item->status == 4 && !$item->historyApprovalTimesheetNote->count())
                                        <td class="delete"><a class="btn btn-danger btn-xs" onclick="hapus_(this)"><i class="fa fa-trash"></i></a></td>
                                        @endif
                                    </tr>
                                    @foreach($item->historyApprovalTimesheetNote as $val)
                                    @if($val && $val->note)
                                    <tr class="approval-note">
                                        <td></td>
                                        <td>NOTE APPROVAL {{ $val->historyApprovalTimesheet->setting_approval_level_id }}</td>
                                        <td colspan="8">
                                            <input type="text" name="transactions[{{ $no }}][note][{{ $val->historyApprovalTimesheet->setting_approval_level_id }}]" readonly class="form-control" value="{{ $val->note }}" />
                                        </td>
                                    </tr>
                                    @endif
                                    @endforeach
                                    ...
                                    <tr>
                                        ...
                                        <td><input type="text" id="activity_name-{{ $no }}" name="transactions[{{ $no }}][timesheet_activity_name]" {{ ($item['status'] == 3 || $item['status'] == 4) && !$item['timesheet_activity_id'] ? '' : 'readonly' }} class="form-control activity_name" value="{{ $item['timesheet_activity_name'] }}"></td>
                                        <td><input type="text" name="transactions[{{ $no }}][description]" {{ $item['status'] == 3 || $item['status'] == 4 ? '' : 'readonly' }} class="form-control" value="{{ $item['description'] }}" /></td>
                                        <td><input type="text" data-index="{{ $no }}" name="transactions[{{ $no }}][date]" {{ $item['status'] == 3 || $item['status'] == 4 ? '' : 'readonly' }} class="form-control date" value="{{ $item['date'] }}" /></td>
                                    ...
                                    @if(isset($item['note']))
                                    @foreach($item['note'] as $key => $val)
                                    @if($val)
                                    <tr class="approval-note">
                                        <td></td>
                                        <td>NOTE APPROVAL {{ $key }}</td>
                                        <td colspan="8">
                                            <input type="text" name="transactions[{{ $no }}][note][{{ $key }}]" readonly class="form-control" value="{{ $val }}" />
                                        </td>
                                    </tr>
                                    @endif
                                    @endforeach
