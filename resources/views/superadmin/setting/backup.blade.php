...
<div id="page-wrapper">
    <div class="container-fluid">
        ...
        <div class="row">
            <form class="form-horizontal" id="form-setting" autocomplete="off" name="form_setting" enctype="multipart/form-data" action="{{ route('superadmin.setting.backup-save') }}" method="POST">
                {{ csrf_field() }}
                                    ...
                                    <tbody>
                                        ...
                                        @foreach(get_schedule_backup() as $key => $item)
                                            <tr>
                                                ...
                                                <td>{{ $item->date }}</td>
                                                <td>
                                                    <a href="{{ route('superadmin.setting.delete-backup-schedule', $item->id) }}" onclick="return confirm('Delete this data?')" class="btn btn-danger btn-xs"><i class="fa fa-trash"></i> delete </a>
                                                </td>
                                            </tr>
                                        @endforeach
                                    </tbody>
            ...
            <div class="col-md-6 p-l-0">
                <div class="white-box">
                    <table class="table table-stripped">
                        ...
                        <tr>
                            <td>
                                <form method="POST" action="{{ route('superadmin.setting.backup-get') }}">
                                    {{ csrf_field() }}
                            ...
                            <td>
                                <form method="POST" action="{{ route('superadmin.setting.backup-delete') }}">
                                    {{ csrf_field() }}
                                    <input type="hidden" name="file" value="{{ $item }}">
...
<div class="modal fade" id="modal_add_schedule_backup" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true" style="display: none;">
    <div class="modal-dialog">
        <div class="modal-content">
          <form method="POST" action="{{ route('superadmin.setting.store-backup-schedule') }}">
             {{ csrf_field() }}
